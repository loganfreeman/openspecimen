...
data-source add --name=mysqlDS --driver-name=mysql --jndi-name=java:jboss/datasources/openspecimen --connection-url=jdbc:mysql://$MYSQL_URI/openspeciment_docker?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=root --password=pass22 --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
...
# Deploy the WAR
cp /opt/jboss/wildfly/customization/openspecimen.war $JBOSS_HOME/$JBOSS_MODE/deployments/openspecimen.war
